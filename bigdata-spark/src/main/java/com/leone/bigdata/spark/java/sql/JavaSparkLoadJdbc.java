...
public class JavaSparkLoadJdbc {
    ...
    public static void main(String[] args) {
        ...
        String url = "jdbc:mysql://ip:3306/boot?useSSL=false";
        ...
        Dataset<Row> dataset = spark.read().jdbc(url, "t_user", props);

        dataset.select(new Column("s_id").as("id"), new Column("s_name").as("name")).where("s_age < 22").distinct().limit(20).show();

        dataset.write().jdbc(url, "t_user_bak1", props);
